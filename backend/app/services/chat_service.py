            ...
            try:
                ...
                async with new_async_session() as db:
                    ...
                    if pp:
                        ...
                        # 查 CameraPreset
                        result = await db.execute(
                            select(CameraPreset).filter(CameraPreset.id == preset_ref)
                        )
                        cp = result.scalar_one_or_none()
                        if cp:
                            # camera 类型预设：注入 camera_params（enabled=true）
                            cam_params = {
                                "enabled": True,
                            ...
                            # 前端摄像机参数优先；AI 未提取且前端没传时用预设
                            if not func_args.get("camera_params") and not frontend_camera_params:
                                func_args["camera_params"] = cam_params
                                logger.info(
                                    "[Chat] preset_ref=%s CameraPreset 已注入 camera_params", preset_ref
                                )
                            elif frontend_camera_params and frontend_camera_params.get("enabled"):
                                # 前端启用了摄像机参数 → 前端优先，预设忽略
                                pass
                            elif not (func_args.get("camera_params") or {}).get("enabled"):
                                func_args["camera_params"] = cam_params
                                logger.info(
                                    "[Chat] preset_ref=%s CameraPreset 已注入 camera_params（AI/前端均未启用）",
                                    preset_ref,
                                )
                            # ── increment usage_count ──
                            cp.usage_count = (cp.usage_count or 0) + 1
                            await db.commit()
                            logger.info(
                                "[Chat] preset_ref=%s (CameraPreset) usage_count 已 +1 → %s",
                                preset_ref, cp.usage_count,
                            )
            except Exception as e:
