...
# =====================================================
# 生图/生视频模型解析链
# - 显式指定 > 用户偏好 default_image_model_id / default_video_model_id > 该类型第一个
# - 与对话模型解析链同构：每一级校验模型仍在注册表中，失效自动落到下一级
# =====================================================

async def resolve_user_media_model_id(db, user_id: int, model_type: str, explicit: str = "") -> str:
    """
    生图/生视频模型解析：显式指定 > 用户偏好（default_{type}_model_id）> 该类型第一个。
    user_id<=0 时跳过用户偏好（匿名/系统上下文）。
    model_type 为 "image" 或 "video"。未命中任何偏好的模型自动落到下一级，
    避免把已停用/下线的模型名发往上游产生隐性费用与失败。
    """
    models = await get_models_by_type(model_type)
    ...
    if user_id:
        ...
        pref = result.scalars().first()
        preferred = ((pref.preferences or {}).get("generation", {}).get(f"default_{model_type}_model_id", "") if pref else "")
        preferred = (preferred or "").strip()
        if preferred and preferred in valid:
            return preferred
    return models[0].id if models else ""
