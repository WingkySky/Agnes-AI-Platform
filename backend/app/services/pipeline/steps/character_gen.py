...
# 角色生成步骤执行器（人物生成）
# 两阶段执行：
#   1. LLM 阶段：根据上游角色清单为每个角色生成详细设定文本（外貌/服装/性格关键词）
#   2. 图片阶段：根据设定文本为每个角色生成参考图
# 按元素独立记录 status，单个角色失败不阻塞其他角色
# =====================================================
...
# 默认角色设定生成 prompt 模板（用户可在 llm_config.prompt_template 中覆盖）
_DEFAULT_CHARACTER_PROMPT_TEMPLATE = """请为以下角色生成详细的设定文本，用于后续生成角色参考图。

角色名称：{{ name }}
角色简述：{{ description }}

请输出一段详细的角色设定描述（150-300字），包含以下方面：
1. 外貌特征（脸型、发型、瞳色、体型、肤色等）
2. 服装打扮（风格、颜色、配饰、标志性装扮等）
3. 性格气质关键词
4. 其他显著特征（如伤疤、纹身、特殊标记等）

直接输出设定文本，不要加标题或分段标记。"""


@register_step_executor
class CharacterGenExecutor(BaseStepExecutor):
    """
    角色生成步骤执行器（人物生成）

    两阶段执行：
    - 阶段一 LLM：从上游读取角色清单，为每个角色生成详细设定文本
    - 阶段二 image_batch：用设定文本作为 prompt 为每个角色生成参考图
    ...
    step_type = "character_gen"

    async def validate(self) -> None:
        """验证输入"""
        config = self.config.get("config", {})
        character_source = config.get("character_source", "")
        if not character_source:
            raise ValueError("缺少 character_source 配置")
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        # 1. 从上游读取角色清单
        characters = self._get_characters(config.get("character_source", ""))
        if not characters:
            logger.warning(f"[角色生成] 上游无角色清单: step_key={self.step_key}")
            return {
                "items": [],
                "summary": {"total": 0, "success_count": 0, "failed_count": 0},
            }

        logger.info(f"[角色生成] 开始生成 {len(characters)} 个角色: step_key={self.step_key}")

        # 2. 阶段一：并发调用 LLM 生成每个角色的设定文本
        llm_results = await self._generate_settings_concurrent(characters, config)

        # 3. 阶段二：并发调用图片 API 生成每个角色的参考图
        ...
        logger.info(
            f"[角色生成] 完成: 成功 {success_count}/{len(items)}, 失败 {failed_count}"
        )
        ...
        if success_items and self.context.run_id:
            try:
                async with new_async_session() as db:
                    ...
                    logger.info(f"[角色生成] 已保存 {len(success_items)} 条生成记录: step_key={self.step_key}")
            except Exception as e:
                logger.warning(f"[角色生成] 保存生成记录失败: {e}")
    ...
    async def execute_single(self, ctx: SingleItemContext) -> ItemResult:
        ...
        logger.info(f"[角色生成-单元素] 重生角色: id={char_id}, name={name}")
        ...
        if prompt_override:
            ...
            try:
                ...
                logger.error(
                    f"[角色生成-单元素] LLM 生成失败: id={char_id}, name={name}, err={e}",
                    exc_info=True,
                )
                return ItemResult(
                    status="failed",
        ...
        # 阶段二：图片生成
        try:
            image_url = await self._generate_character_image(
            ...
            logger.error(
                f"[角色生成-单元素] 图片生成失败: id={char_id}, name={name}, err={e}",
                exc_info=True,
            )
            return ItemResult(
                status="failed",
    ...
    def _get_characters(self, source_path: str) -> List[Dict[str, Any]]:
        """
        从上游步骤输出中读取角色清单。

        支持点路径访问，如 "steps_output.step_storyboard.characters"
        或 "step_storyboard.characters"（默认从 steps_output 开始）。
        若上游没有 characters 字段或为空，返回空列表（不报错）。
        """
        if not source_path:
            return []
        ...
        for idx, item in enumerate(current):
            ...
            if not name:
                ...
                continue
            characters.append({
                "id": item.get("id") or f"char_{idx + 1:03d}",
                "name": name,
                "description": description,
            })
        return characters
    ...
    async def _generate_settings_concurrent(
        ...
        """阶段一：并发调用 LLM 生成每个角色的设定文本"""
        llm_config = config.get("llm_config", {})
        max_concurrent = llm_config.get("max_concurrent", 3)
        semaphore = asyncio.Semaphore(max_concurrent)
        results: List[Dict[str, Any]] = [{}] * len(characters)

        async def gen_one(idx: int, char: Dict[str, Any]) -> None:
            async with semaphore:
                char_id = char.get("id", f"char_{idx + 1:03d}")
                name = char.get("name", "")
                ...
                try:
                    ...
                    logger.error(
                        f"[角色生成] LLM 阶段失败: id={char_id}, name={name}, err={e}",
                        exc_info=True,
                    )
                    results[idx] = {
                        "id": char_id,
        ...
        await asyncio.gather(
            *(gen_one(i, c) for i, c in enumerate(characters))
        )
        return results
    ...
    async def _generate_images_concurrent(
        ...
        async def gen_one(idx: int, llm_res: Dict[str, Any]) -> None:
            async with semaphore:
                char_id = llm_res.get("id", f"char_{idx + 1:03d}")
                name = llm_res.get("name", "")

                # LLM 失败的角色直接标记失败，不进入图片阶段
                ...
                try:
                    ...
                    logger.error(
                        f"[角色生成] 图片阶段失败: id={char_id}, name={name}, err={e}",
                        exc_info=True,
                    )
                    results[idx] = {
                        "id": char_id,
    ...
    async def _generate_setting_text(
        ...
        """调用 LLM 为单个角色生成详细设定文本"""
        llm_config = config.get("llm_config", {})
        system_prompt = llm_config.get(
            "system_prompt",
            "你是一位专业的角色设计师，擅长创作生动、独特的角色设定。",
        )
        prompt_template = (
            llm_config.get("prompt_template") or _DEFAULT_CHARACTER_PROMPT_TEMPLATE
        )
    ...
    async def _generate_character_image(
        ...
        """调用图片生成 API 为单个角色生成参考图（文生图模式）"""
        image_config = config.get("image_config", {})
        size = image_config.get("size", "768x1024")

        # 构建图片 prompt：角色名 + 设定文本
        prompt = f"角色参考图 - {name}: {setting_text}"
        ...
        if not model:
            try:
                ...
                logger.warning(f"[角色生成] 获取可用图片模型失败: {e}")
                model = ""
