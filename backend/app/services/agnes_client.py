...
class AgnesAIClient:
    ...
    async def create_image(
        ...
        # ── 参考图按模型生成能力配置截断（model_definitions.gen_params / 自动画像，
        #    如 2.1 Flash 上游最多 6 张，超限会被上游 400 拒绝），其余模型原样透传；
        #    保留前 N 张，调用方已按「锚点/底图 > 角色 > 场景」优先级排序 ──
        from app.services.provider_registry import provider_registry
        _ref_limit = (await provider_registry.get_model_gen_params(model)).max_ref_images
