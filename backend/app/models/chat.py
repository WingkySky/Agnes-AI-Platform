...
class ChatSession(Base):
    ...
    # 会话类型：chat=全局对话页；canvas=画布 Agent 会话（前端内核驱动，context 列存 LLM 上下文）
    session_type = Column(String(20), nullable=False, default="chat", index=True, comment="会话类型：chat / canvas")
    # 画布会话关联的画布工作区 id（跳转定位用；chat 类型为 NULL）
    workspace_id = Column(String(64), nullable=True, index=True, comment="画布工作区 id（仅 canvas 会话）")
    # 画布会话的 LLM 上下文（内核 serializeState 原样，每轮全量覆盖；恢复唯一可信源）
    context = Column(JSON, nullable=True, comment="内核序列化状态（仅 canvas 会话）")
...
class ChatMessage(Base):
    ...
    # 画布 Agent 工具步骤时间线（仅 canvas 会话：[{callId, tool, args, status, result}]）
