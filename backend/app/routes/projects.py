...
#   3. 向导:                POST /projects/wizard（仅建草稿；4 步 LLM 编排已迁前端）
#   4. SSE:                 GET /projects/{id}/events
#   5. 剧本:                /projects/{id}/scripts[/{sid}]
#   6. 角色/场景/道具:      /projects/{id}/characters|scenes|props[/{eid}]
#                           + 子路由 generate-image / batch-generate / versions / set-active / upload
...
from app.services.project.frame_image_service import (
    list_frame_images, get_frame_image, upload_frame_image,
...
@router.post("/wizard", summary="通过模板向导创建项目（仅建草稿）")
async def create_via_wizard_api(
    ...
    """
    创建向导草稿项目（status=creating）。

    4 步 LLM 编排（剧本→实体→分镜→帧提示词）已迁前端 lib/storyboard 管线，
    由 ProjectLaunchDialog 本地逐步执行并通过 CRUD 端点落库；本端点只负责建项目。
    """
    ...
    # 把 category 写入 wizard_inputs，便于"重新生成剧本"等后续编排找回模板类别
...
def _build_media_routes(
    *,
    ...
    generate_schema: Optional[type] = None,  # 生成路由（帧图已迁前端 images/tasks，仅视频/音频保留）
    generate_call=None,                     # async (db, shot_id, user_id, data) -> result
