 ...
 */
async function imageToBase64(source: string): Promise<string> {
  if (!source) {
...
/** 各视频节点最近播放位置（面板 id → currentTime 秒），供"截取当前帧"使用 */
const videoCurrentTimes = new Map<string, number>()

/** 记录视频节点最近播放位置（CanvasNode 的 timeupdate 时调用） */
export function registerVideoTime(panelId: string, time: number): void {
  videoCurrentTimes.set(panelId, Number(time) || 0)
}

/** 读取视频节点最近播放位置（无记录时返回 0 = 首帧） */
export function getVideoTime(panelId: string): number {
  return videoCurrentTimes.get(panelId) || 0
}

/** 截帧时间：数字为秒；'first'=首帧，'last'=尾帧（回缩 0.05s 避免 seek 到 duration 失败/黑帧） */
export type VideoFrameTime = number | 'first' | 'last'

/** 加载视频并 seek 到指定时间，返回可绘制帧的 video 元素 */
function loadVideoAt(source: string, time: VideoFrameTime): Promise<HTMLVideoElement> {
  return new Promise((resolve, reject) => {
    ...
    video.preload = 'auto'
    let settled = false
    const done = (err: Error | null, el?: HTMLVideoElement) => {
      if (settled) return
      settled = true
      window.clearTimeout(timer)
      err ? reject(err) : resolve(el!)
    }
    const timer = window.setTimeout(() => done(new Error('视频加载超时')), 20000)
    video.onloadedmetadata = () => {
      if (time === 'last') {
        video.currentTime = Math.max(0, video.duration - 0.05)
      } else if (time === 'first') {
        // currentTime 已是 0 时再赋 0 不会触发 seeked，抬到 0.001s
        video.currentTime = 0.001
      } else {
        video.currentTime = time <= 0 ? 0.001 : Math.min(time, Math.max(0, video.duration - 0.05))
      }
    }
    video.onseeked = () => done(null, video)
    video.onerror = () => done(new Error('视频加载失败'))
    video.src = source
  })
}

/** 将 video 当前帧绘制为 JPEG data URI */
function videoToDataUrl(video: HTMLVideoElement): string {
  const canvas = document.createElement('canvas')
...
/**
 * 从视频截取一帧，返回 JPEG data URI
 * - 优先直连加载（CDN 带 CORS 头时可直接截帧）
 * - 直连失败（跨域污染/加载失败）时自动回退后端 /api/download 代理（同源）
 * @param time - 目标时间（秒）；传节点最近播放位置即"当前帧"，0 或 'first' 为首帧，'last' 为尾帧
 */
export async function captureVideoFrame(source: string, time: VideoFrameTime): Promise<string> {
  if (!source) {
    throw new Error('captureVideoFrame: source 为空')
  }
  try {
    return videoToDataUrl(await loadVideoAt(source, time))
  } catch (directErr) {
    if (!source.startsWith('http')) throw directErr
    const proxied = `/api/download?url=${encodeURIComponent(source)}&type=video`
    return videoToDataUrl(await loadVideoAt(proxied, time))
  }
}
