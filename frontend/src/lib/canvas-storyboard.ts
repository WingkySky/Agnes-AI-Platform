...
import { buildFramePrompt } from '@/lib/storyboard/prompts'
  ...
  for (const item of raw) {
    ...
    shots.push({
      ...
      props: Array.isArray(item.props)
        ? item.props.filter((c): c is string => typeof c === 'string')
        : [],
...
export function readAssets(panel: { content?: Record<string, unknown> }): ScriptAssets {
  const out: ScriptAssets = { characters: [], scenes: [], props: [] }
...
export function mergeExtractedAssets(
  ...
  extracted?: {
    characters?: Array<{ name?: unknown; description?: unknown }>
    scenes?: Array<{ name?: unknown; description?: unknown }>
  ...
  const props = pick(current.props, extracted?.props)
...
/** 某资产卡在分镜中的出场镜头号（角色按 characters 含名、物品按 props 含名，场景按 location 同名） */
export function shotNosForAsset(shots: CanvasShot[], kind: 'characters' | 'scenes' | 'props', name: string): number[] {
  const trimmed = name.trim()
  if (!trimmed) return []
  return shots
    .filter((s) => (kind === 'characters' ? s.characters.includes(trimmed) : kind === 'props' ? s.props.includes(trimmed) : s.location.trim() === trimmed))
    .map((s) => s.no)
}
...
export function buildShotContexts(shot: CanvasShot, assets: ScriptAssets, extraCharacters: string[] = []): ShotContexts {
  ...
  const shotProps = shot.props.map((c) => c.trim()).filter(Boolean)
  ...
  const hitCharacters = shotNames.length > 0
    ...
    : assets.characters
  const hitProps = shotProps.length > 0
    ? assets.props.filter((a) => shotProps.includes(a.name.trim()))
    : []
  ...
  const hitPropTexts = hitProps.map(assetText).filter(Boolean)
  ...
  return {
    ...
    propImages: hitProps.map((a) => a.imageUrl).filter(Boolean),
  }
}
...
async function deriveImagesInternal(scriptPanel: CanvasPanel, items: PendingFrame[]): Promise<void> {
  ...
  for (const item of items) {
    ...
    const baseRefs = [...upstreamRefImages, ...contexts.characterImages, ...contexts.sceneImages, ...contexts.propImages].filter(Boolean)
    const promptLines = [buildFramePrompt(shot, contexts, store.activeStyleConfig)]
    ...
    const referenceImages = item.anchorUrl
      ? [item.anchorUrl, ...contexts.characterImages, ...contexts.sceneImages, ...contexts.propImages].filter(Boolean)
      : item.linkUrl
...
export async function deriveChainVideosForShot(scriptPanel: CanvasPanel, shot: CanvasShot): Promise<void> {
  ...
  const x = baseX + (idx % GRID_COLS) * IMG_PITCH_X
  const contexts = buildShotContexts(shot, readAssets(scriptPanel))
  const referenceImages = [firstUrl, ...contexts.characterImages, ...contexts.sceneImages, ...contexts.propImages].filter(Boolean)
  ...
  for (const item of fillItems) {
    ...
    const promptLines = [
      buildFramePrompt(shot, contexts, store.activeStyleConfig),
