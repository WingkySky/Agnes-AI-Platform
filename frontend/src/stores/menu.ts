 ...
 *   - 从后端拉取用户自定义配置（显示/位置/排序/分组/分组名称）覆盖默认
 *   - 本地缓存配置（localStorage，避免每次刷新都请求）
 *   - 管理员：菜单配置管理操作
 *
 * 设计理念：
 *   - 所有菜单项在前端固定定义（key、名称、图标、路径、权限）
 *   - 用户只需要配置：哪些显示、放顶部还是侧边栏、排序、分组、分组名称图标
 *   - 完全没有代码化字段需要用户填写
 * ===================================================== */
...
import {
  ...
  saveMenuConfigs,
  resetMenuConfigs,
  type MenuItemConfig as ApiMenuItemConfig,
  type MenuGroupConfig as ApiMenuGroupConfig,
} from '@/api/menu'
import {
  DEFAULT_MENU_CONFIG,
  resolveMenus,
  getAllMenuItemsForAdmin,
  getTopNavGroupsForAdmin,
...
export const useMenuStore = defineStore('menu', () => {
  ...
  /** 完整菜单配置数据（包含菜单项和分组配置） */
  const fullMenuConfig = computed<MenuConfigData>(() => ({
    items: menuConfigs.value,
    groups: groupConfigs.value,
  }))

  /** 解析后的顶部导航菜单 */
  const topNav = computed(() => {
    const { topNav } = resolveMenus(fullMenuConfig.value, isAdmin.value, locale.value)
    return topNav
  })

  /** 解析后的侧边栏菜单（已按分组整理） */
  const sidebar = computed(() => {
    const { sidebar } = resolveMenus(fullMenuConfig.value, isAdmin.value, locale.value)
    return sidebar
  })
  ...
  const allMenuItems = computed<AdminMenuItem[]>(() => {
    return getAllMenuItemsForAdmin(fullMenuConfig.value)
  })

  /** 所有顶部导航分组（管理界面用，包含自定义配置） */
  const allTopNavGroups = computed<AdminMenuGroup[]>(() => {
    return getTopNavGroupsForAdmin(groupConfigs.value)
  })

  /** 所有侧边栏分组（管理界面用，包含自定义配置） */
  const allSidebarGroups = computed<AdminMenuGroup[]>(() => {
    return getSidebarGroupsForAdmin(groupConfigs.value)
  })
  ...
  function loadFromCache(): boolean {
    try {
      ...
      if (cached) {
        ...
        if (wrapper.version === MENU_CACHE_VERSION) {
          if (wrapper.data && Array.isArray(wrapper.data.items)) {
            // 新版缓存格式
            menuConfigs.value = wrapper.data.items
            groupConfigs.value = wrapper.data.groups || []
  ...
  /** 批量保存菜单配置（包含菜单项和分组配置） */
  async function saveConfigs(configs: MenuItemConfig[], groups: MenuGroupConfig[] = []) {
    try {
      await saveMenuConfigs({ configs: configs as ApiMenuItemConfig[], groups: groups as ApiMenuGroupConfig[] })
  ...
  async function updateItemConfig(key: string, updates: Partial<MenuItemConfig>) {
    ...
    await saveConfigs(newConfigs, groupConfigs.value)
  }
