...
/** 工作区级风格选择：库条目（presetId）或自定义文本，config 为合并后的生效配置 */
export interface CanvasStyleSelection {
  presetId: number | null
  /** 自定义风格文本（未选库条目时生效） */
  customText: string
  /** 生效风格配置（库条目 prompt_config 或 customText 渲染） */
  config: StyleConfig
}
...
export const useCanvasStore = defineStore('canvas', {
  ...
  getters: {
    ...
    /** 当前工作区的生效风格配置（未选择时为空风格：管线跳过风格段） */
    activeStyleConfig(state): StyleConfig {
      const ws = state.workspaces.find((w) => w.id === state.activeWorkspaceId)
      return ws?.styleConfig?.config ?? { prefix: '', suffix: '', negativePrompt: '' }
    },
    ...
    /**
     * 获取接入某个目标节点的源资源节点数组（带序号，按连接创建顺序排序，去重）
     * - 委托 lib/canvas-generation 的唯一实现（getUpstreamNodesWithIndex）
     * - 仅保留 image/text/video/audio 资源节点，序号与生成时 @[图片N]/@[文本N] 标签编号一致
     */
  ...
  actions: {
    ...
    /** 切换背景模式：'dots' | 'lines' | 'blank'（dots/lines/blank） */
    ...
    /** 设置当前工作区的风格选择（null 清空；写入工作区对象随画布持久化） */
