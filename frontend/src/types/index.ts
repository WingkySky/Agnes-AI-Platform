...
export interface ModelGenParams {
  ...
  /** 该模型支持的时长档位（秒，视频生成）；null=用全局 video_durations */
...
export interface ImageGenerationRequest {
  ...
  /** 摄像机参数（enabled=true 时后端拼接到 prompt 末尾） */
...
export interface ChatSession {
  id: number
  title: string
  /** 会话类型：chat=对话页；canvas=画布 Agent 会话 */
  session_type?: 'chat' | 'canvas'
  /** 画布会话关联的画布工作区 id */
...
export interface AgentSessionSyncPayload {
  ...
  messages: Array<{
    ...
    /** 生成产物（写消息行 media_items 列） */
    media_items?: Array<{ type: string; url: string; task_id?: string; status: string }>
  }>
}
