...
const tools = computed(() => {
  ...
  // 2.5 分镜派生结果节点：图生视频 / 生成尾帧 / 推演前段画面 / 生成分段视频 / 重拍此镜头
  ...
  // video 有内容：截取首帧 / 当前帧 / 尾帧 → 生成图片子节点
  if (isVideo.value && hasContent.value) {
    list.push({
      id: 'capture-frame-first',
      title: t('canvas.hoverToolbar.captureFrameFirst'),
      icon: SkipBack,
      onClick: () => emit('capture-frame', { panel: props.panel, position: 'first' }),
    })
    list.push({
      id: 'capture-frame',
      title: t('canvas.hoverToolbar.captureFrame'),
      icon: Film,
      onClick: () => emit('capture-frame', { panel: props.panel, position: 'current' }),
    })
    list.push({
      id: 'capture-frame-last',
      title: t('canvas.hoverToolbar.captureFrameLast'),
      icon: SkipForward,
      onClick: () => emit('capture-frame', { panel: props.panel, position: 'last' }),
    })
  }
