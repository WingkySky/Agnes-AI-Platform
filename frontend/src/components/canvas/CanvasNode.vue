    ...
    >
      ...
      <div class="node-content-wrap" :style="contentWrapStyle">
        ...
        <div v-else-if="panel.type === 'image'" class="image-content">
          <!-- 实体设定卡（content.kind 标记）：徽标 + 点击展开描述编辑小窗 -->
          <template v-if="entityKind">
            <div
              class="entity-badge" :style="{ background: theme.toolbar.activeBg, color: theme.node.text }"
              @click.stop="entityEditorOpen = !entityEditorOpen"
            >
 ...
 * CanvasNode 画布节点组件
 *
 * 数据约定：panel.content 作为节点元数据（节点元数据）
...
const inputBadges = computed(() => {
  ...
  // 查找当前节点作为 source 的所有下游连接；config/script 等非资源节点不参与连线收集，天然无标记
  const outgoingConns = store.connections.filter(c => c.source_panel_id === props.panel.id)
  for (const conn of outgoingConns) {
    const targetId = conn.target_panel_id
    const targetPanel = store.panels.find(p => p.id === targetId)
    if (!targetPanel || !targetId) continue

    // 获取该目标节点的输入序号（与生成时 @[图片N] 标签编号同源，来自 lib 的统一实现）
    const idx = store.getInputNodesWithIndex(targetId)
...
const entityKind = computed(() => {
  ...
  return k === 'character' || k === 'scene' || k === 'prop' ? k : null
})
const entityEditorOpen = ref(false)
const entityKindLabel = computed(() =>
  entityKind.value === 'character' ? t('canvas.node.entityCharacter')
...
/** 播放位置变化：记录最近时间，供"截取当前帧"使用 */
function onVideoTimeUpdate(e: Event) {
  registerVideoTime(props.panel.id, (e.target as HTMLVideoElement).currentTime)
}
