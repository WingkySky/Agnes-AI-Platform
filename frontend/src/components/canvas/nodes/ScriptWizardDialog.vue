<!-- =====================================================
  ScriptWizardDialog 分镜向导（无限画布 script 节点）
  ...
  <Teleport to="body">
    <div v-if="visible && panel" class="wizard-overlay" :style="overlayStyle">
      ...
      <div v-else-if="step === 2" class="wizard-body">
        <!-- 全局风格（画布级：库条目或自定义文本，实体图/分镜图统一取用） -->
        <div class="style-row">
          <span class="style-label" :style="accentStyle">{{ t('canvas.script.wizard.globalStyle') }}</span>
          <select v-model="styleChoice" class="wiz-input style-select" :style="inputStyle" @change="onStyleChange">
            <option value="none">{{ t('canvas.script.wizard.styleNone') }}</option>
            <option v-for="s in styleEntries" :key="s.id" :value="String(s.id)">{{ s.name }}</option>
            <option value="custom">{{ t('canvas.script.wizard.styleCustom') }}</option>
          </select>
          <input
            v-if="styleChoice === 'custom'"
            v-model="styleCustom"
            class="wiz-input style-input" :style="inputStyle"
            :placeholder="t('canvas.script.stylePlaceholder')"
            @input="onStyleChange"
          >
        </div>
...
const uploadTarget = ref<{ kind: 'characters' | 'scenes' | 'props'; id: string } | null>(null)
...
async function onStyleChange() {
  const presetId = /^\d+$/.test(styleChoice.value) ? Number(styleChoice.value) : null
  const customText = styleChoice.value === 'custom' ? styleCustom.value.trim() : ''
  const config = presetId != null
    ? (await resolveStyleConfig(presetId)) ?? { prefix: '', suffix: '', negativePrompt: '' }
    : { prefix: customText, suffix: '', negativePrompt: '' }
  store.setWorkspaceStyleConfig({ presetId, customText, config })
}
...
const assetSections = computed(() => [
  ...
  { kind: 'props' as const, list: assets.value.props, title: t('canvas.script.wizard.props'), addLabel: t('canvas.script.wizard.addProp') },
])
...
function assetShotLabel(kind: 'characters' | 'scenes' | 'props', name: string): string {
...
/** 生成资产参考图（角色/场景/物品设定图：名称 + 描述 + 画布级风格，模板在管线 prompts） */
async function generateAssetImage(asset: ShotAsset, kind: 'characters' | 'scenes' | 'props') {
...
async function generateAllAssets() {
  ...
  for (const asset of pending) {
    const kind = assets.value.characters.includes(asset)
      ? 'characters'
      : assets.value.scenes.includes(asset) ? 'scenes' : 'props'
    await generateAssetImage(asset, kind)
  }
}

/** 本地上传参考图（dataURL 存节点 content，localforage 持久化） */
function openUpload(asset: ShotAsset, kind: 'characters' | 'scenes' | 'props') {
...
const shotPrompts = computed(() => {
  ...
  for (const shot of shots.value) {
    ...
    map.set(shot.id, buildFramePrompt(shot, contexts, store.activeStyleConfig))
  }
  return map
})
