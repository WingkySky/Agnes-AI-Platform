...
<template>
  ...
  >
    ...
    <!-- 步骤3：创建 + 本地编排进度（4 步 LLM 链在前端执行） -->
    <template v-if="step === 3">
      <div class="launch-section creating">
        <div class="orch-steps">
          <div v-for="(s, idx) in stepStates" :key="s.key" class="orch-step" :class="`is-${s.status}`">
            <div class="orch-icon">
              <el-icon v-if="s.status === 'done'" class="is-done"><Check /></el-icon>
              <el-icon v-else-if="s.status === 'error'" class="is-error"><Close /></el-icon>
              <el-icon v-else-if="s.status === 'running'" class="creating-icon small"><Loading /></el-icon>
              <span v-else class="orch-index">{{ idx + 1 }}</span>
            </div>
            <div class="orch-info">
              <div class="orch-label">{{ s.label }}</div>
              <div v-if="s.error" class="orch-error">{{ s.error }}</div>
            </div>
          </div>
        </div>
        <p v-if="!failed" class="creating-text">AI 正在按步骤创建项目内容，请稍候...</p>
        <p v-else class="creating-text">编排中断：已生成的内容会自动跳过，可直接重试</p>
      </div>
    </template>
    ...
    <template #footer>
      ...
      <el-button v-if="step === 3 && failed" type="primary" :loading="creating" @click="runSteps">
        重试
      </el-button>
    </template>
  </el-dialog>
...
import { Film, Loading, Check, Close } from '@element-plus/icons-vue'
import { useProjectStore, type WizardStepState } from '@/stores/project'
...
const stepStates = ref<WizardStepState[]>([])
const failed = computed(() => stepStates.value.some((s) => s.status === 'error'))
...
async function launch() {
  ...
  step.value = 3
  try {
    // ProjectLaunchDialog 的 4 种场景对应原 wizard_chains 四类模板；
    // 后端只建草稿项目，4 步 LLM 编排在下方前端管线中逐步执行
    ...
    // 步骤错误已在进度面板标红，可点"重试"（编排幂等续跑）
  } finally {
    creating.value = false
  }
}

/** 执行四步编排（重试入口；已有产物自动跳过） */
async function runSteps() {
  if (!createdProjectId.value) return
  await projectStore.runWizardOrchestration(createdProjectId.value, (steps) => {
    stepStates.value = steps
  })
}
</script>
...
.launch-section.creating { text-align: center; padding: 24px 0; }
.creating-icon { color: var(--el-color-primary); animation: rotating 2s linear infinite; }
.creating-icon.small { font-size: 18px; }
.creating-text { margin-top: 16px; color: var(--el-text-color-secondary); }
.orch-steps { display: flex; flex-direction: column; gap: 10px; text-align: left; max-width: 380px; margin: 0 auto; }
.orch-step { display: flex; align-items: flex-start; gap: 10px; }
.orch-icon { width: 24px; display: flex; justify-content: center; padding-top: 1px; }
.orch-icon .is-done { color: var(--el-color-success); }
.orch-icon .is-error { color: var(--el-color-danger); }
.orch-index { color: var(--el-text-color-secondary); font-size: 13px; }
.orch-label { font-size: 14px; }
.orch-step.is-error .orch-label { color: var(--el-color-danger); }
.orch-error { color: var(--el-color-danger); font-size: 12px; margin-top: 2px; }
