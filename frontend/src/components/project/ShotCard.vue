...
// 景别/运镜值由分镜管线按词表产出（中文直出），不再做遗留英文键映射
const shotTypeLabel = computed(() => props.shot.shot_type || '')

const cameraMovementLabel = computed(() => props.shot.camera_movement || '')
...
async function onGeneratePrompt() {
  ...
  try {
    await projectStore.rebuildShotPrompt(props.shot.id)
    ElMessage.success('提示词已按当前设定重建')
    emit('refresh')
  } catch (e: any) {
    ElMessage.error(e?.message || '生成失败')
  } finally {
