  ...
  <div class="shots-tab">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <el-select
          :model-value="projectStore.currentProject?.style_config?.preset_id ?? null"
          placeholder="画面风格（不设置）"
          clearable
          filterable
          style="width: 200px"
          @change="onStyleChange"
        >
          <el-option v-for="s in styleEntries" :key="s.id" :label="s.name" :value="s.id" />
        </el-select>
        <el-button :icon="MagicStick" :loading="rebuildingPrompts" :disabled="shots.length === 0" @click="onRebuildAllPrompts">
          按风格重写提示词
        </el-button>
      </div>
...
import { listStyleEntries } from '@/lib/storyboard/library'
...
// ---------- 画面风格（项目级，接 prompt_presets 能力库） ----------
const styleEntries = ref<Array<{ id: number; name: string }>>([])
const rebuildingPrompts = ref(false)

async function onStyleChange(presetId: number | null) {
  try {
    await projectStore.setProjectStyle(presetId || null)
    ElMessage.success(presetId ? '风格已保存，可点击「按风格重写提示词」应用到全部分镜' : '已清空风格')
  } catch (e: any) {
    ElMessage.error(e?.message || '风格保存失败')
  }
}

async function onRebuildAllPrompts() {
  try {
    await ElMessageBox.confirm(
      '将按当前项目风格重建全部分镜的画面提示词（覆盖现有提示词），是否继续？',
      '按风格重写提示词',
      { type: 'info', confirmButtonText: '开始重写', cancelButtonText: '取消' },
    )
  } catch (_) { return }
  rebuildingPrompts.value = true
  try {
    await projectStore.rebuildAllShotPrompts()
    ElMessage.success('已按风格重写全部分镜提示词')
  } catch (e: any) {
    ElMessage.error(e?.message || '重写失败')
  } finally {
    rebuildingPrompts.value = false
  }
}

onMounted(async () => {
  try {
    styleEntries.value = await listStyleEntries()
  } catch { /* 库不可用时选择器留空 */ }
})
