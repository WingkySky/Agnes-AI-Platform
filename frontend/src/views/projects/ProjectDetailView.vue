...
<template>
  <div v-loading="projectStore.currentLoading && !projectStore.currentProject" class="project-detail-view">
    ...
    <template v-else-if="project">
      ...
      <!-- 创建未完成面板：status=creating 时展示（创建对话框中途关闭的草稿） -->
      <div v-if="project.status === 'creating'" class="wizard-panel">
        <div class="wizard-panel-header">
          <h3 class="wizard-title">项目创建未完成</h3>
          <p class="wizard-desc">可继续执行 AI 编排；已生成的剧本/实体/分镜会自动跳过</p>
        </div>
        <el-button type="primary" :loading="resuming" @click="onResumeOrchestration">
          继续创建
        </el-button>
      </div>
...
// ---------- 继续创建：对 creating 草稿重跑前端编排（幂等跳过已有产物） ----------
async function onResumeOrchestration() {
  if (!projectIdRef.value) return
  resuming.value = true
  try {
    await projectStore.runWizardOrchestration(projectIdRef.value)
    await projectStore.fetchProject(projectIdRef.value)
    ElMessage.success('项目创建完成')
  } catch (e: any) {
    ElMessage.error(e?.message || '编排失败，请重试')
  } finally {
