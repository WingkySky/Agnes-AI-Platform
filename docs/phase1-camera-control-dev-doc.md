...
> **数据源迁移说明**：独立 `camera_presets` 表与 `/api/camera-presets` 端点已删除；摄像机/运镜预设统一存于 `prompt_presets`（type=camera，`camera_params` JSON 字段），前端选择器走统一预设广场数据源，画布 CanvasCameraPanel 同源。`camera_prompt.build_camera_prompt_suffix` 拼接函数保留不变。
