...
视频不要超过 15 秒，否则有失败概率。官方建议的时长与帧率组合：
...
前端 UI 与后端均应按此限制联动：选 30 FPS 时不可选 15s；选 60 FPS 时只可选 ≤ 5s。

#### 7. 文本模型开启 Thinking

> 来源：Agnes 官方 Q&A

`chat_template_kwargs` 方式（OpenAI 风格）：
...
免费用户 RPM 20（每分钟 20 次请求，按账号统计），无其他限制。
