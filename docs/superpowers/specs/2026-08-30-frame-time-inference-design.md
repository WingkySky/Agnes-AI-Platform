...
LibTV 于 2026 年 3 月上线"画面时间推演"功能：基于一张关键画面，自动推演生成**前 3 秒 / 后 5 秒的画面延展**，输出的是**画面（图片）而非视频，用于构建更连贯的叙事链，在故事板预演与长视频分段生产中价值最高。需与"剧情推演四宫格"（推演剧情走向的宫格图）区分：画面时间推演是**时间轴方向的延展。
