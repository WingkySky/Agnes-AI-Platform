...
- **「自动」= 用户偏好**：两页进入默认选「自动」——比例=参考图最接近预设（图生，多图取第一张、换图重适配）或偏好「默认比例」（文生）；档位一律取偏好新增的「默认清晰度」（图片 sd/hd/4k）与「默认分辨率」（视频 480p~4K）；比例与档位可分别处于自动/手动；上一轮的最近档位锚定逻辑删除，语义单一化
- **偏好中心**：generation 新增 `default_image_tier`（默认 sd）、`default_video_resolution`（默认 720）两偏好项，后端 DEFAULT_PREFERENCES 同步
- **画布**：参数条复用 ParamSelector 自动获得统一面板，显式关闭自动选项；content 持久化与 canvas-generation 解析零改动
- **提交前解析**：auto 仅为前端状态，提交/积分预估前解析为具体尺寸/比例（width/height 8 对齐逻辑不变），后端生成协议零改动；未上传参考图时回退偏好值
- **测试**：新增 model-params 单测（档位过滤/偏好比例解析），vitest 257 例全绿；后端 pytest 58 过 1 失败（test_audio_clip_cmd 为 HEAD 既有失败：_audio_clip_cmd 已改 async 而测试未同步，与本次无关）

### 图生图/图生视频：「自动」比例选项
- **新增「自动」比例**：图生图尺寸选择与图生视频画幅比新增「自动」档——跟随参考图宽高比（多图取第一张），后续换图自动重新适配；模型无对应比例时按参考图宽高比自动选最接近的预设档位
- **智能锚定**：图片「自动」在最近所选清晰度档（标清/超清/4K）内匹配比例，不再因适配降档；未选「自动」时手动选择不被参考图上传覆盖（未手动选过则保留原有自动适配习惯）
- **提交前解析**：'auto' 仅存前端状态，提交/积分预估/结果展示统一解析为具体尺寸或比例，后端协议零改动；未上传参考图时回退默认档
- **入口**：`RatioPicker` 新增 auto 选项位（图生图尺寸面板顶部 + 图生视频比例首位，虚线框+魔法棒标识），`ParamSelector` 经 `enableAuto` 透传（仅图生图/图生视频模式显示，画布等场景不受影响）；i18n 中英同步

### 用户记忆库：创作偏好的自动沉淀与免查找注入（单一记忆库方案）
- **单一记忆库**：不建新存储——创作偏好就是用户 MCP 记忆图谱里「创作偏好」实体的 observations（格式"类别：内容"），per_user 隔离天然按用户分开；通用事实照旧记普通实体，同一张图谱互不干扰
- **写入靠守则**：系统提示追加「用户创作偏好」段——注入已有偏好的同时带记忆守则（对话中确认稳定偏好时主动 `add_observations` 记入，格式约定 + 不重复 + 用户叫停即停），Agent 用现成 memory 工具完成写入，零新工具零新管线
- **读取免查找**：会话建立时网关以该用户身份 `read_graph` 抽取偏好拼摘要注入系统提示，Agent 开局即知用户偏好，无需任何工具调用；与能力摘要并行拉取（画布单块 async 组合、对话页 `_applySkillsPrompt` 单点组合），未安装记忆服务器静默降级
- **管理闭环**：`GET/DELETE /api/mcp/memory/*` 登录级端点（摘要/单条删/清空实体）；画布面板新增「记忆」胶囊（数量角标+偏好列表+单条删除+一键清空+空态引导），仅记忆服务器可用时显示
- **后端**：`services/memory_service.py`（定位 official-memory 服务器 + 图谱守卫解析 + 网关调用复用，故障不阻断会话）
- **测试**：后端 pytest 5 例（服务器定位/图谱抽取/坏图谱容错/删除清空路由/无服务器降级），前端 vitest 245 例全绿
...
- **门全前移父内核，杜绝档位旁路**：内核门决策抽成统一入口 `authorizeTool`（父工具循环与子代理门请求共用），子代理内部工具与父完全同套三档策略——readonly 拒、confirm 过门、auto 直通；门状态贯穿整个父回合（批量 5 个分镜子代理同阶段只弹 1 次门）；挂起确认改 FIFO 队列，并行子代理撞门按序唤醒，确认卡带「来自子任务」来源标签
- **步骤行实时进度**：子代理运行中父时间线该步骤实时显示「子任务运行中 · 第 N 回合 · 当前工具」，结束后回填结论摘要；进度经 delegate 进度 sink（按父内核路由，多会话/双宿主安全）直写 store，不加面板结构
- **停止传导**：父 requestStop 遍历活跃子内核注册表逐一中止，无悬挂；父流程已停止后子工具直接拒执行防僵尸子循环
- **内核配套扩展**：maxTurns 可配（子 20/父 40）、HostTool/AgentTool execute 透传机制层 callId 与父内核实例、confirm_request 事件增 source 字段；i18n 中英同步
- **测试**：内核门扩展（FIFO/来源标签/门状态共享/停止传导/callId）+ 子代理执行器（基本委派/门前移/readonly 拒绝/禁递归收窄/摘要截断/入参守卫/并发排队）新增 14 例，vitest 232 例全绿
